...
**<https://github.com/sosalejandro/grunnr/security/advisories/new>**
...
demonstrate it, the grunnr version (`grunnr --version`), and the platform. If
you are unsure whether something counts, report it — deciding that is our
job, not yours.

## What to expect

Grunnr is maintained by a small team. These are intentions, not contractual
...
branches: if you are running an older tag, the remedy is to upgrade. Grunnr is
...
Grunnr is a local CLI. It reads your source code, writes a local SQLite
...
- Anything that makes grunnr write outside the paths documented in
...
- Command injection through a repository grunnr scans, a config file, a
  coverage report, or a flag value. On its own initiative grunnr launches
  `git`, `go`, `node` and `python`; `grunnr cov run -- <command>` additionally
  ...
  vulnerability.
- Any path by which grunnr transmits data off the machine.
- Escapes from the MCP server's read-only boundary (`grunnr mcp`): a tool call
...
- SQL injection into the state database from indexed content — a symbol
  name, a file path, or query text that changes what grunnr executes.
- A defect in `grunnr security` that causes it to *understate* what the
  database holds, what leaves it, or what grunnr writes and launches: a table
...
- Anything that makes `grunnr security` (or `grunnr security redact --dry-run`)
...
- Credentials grunnr found in your own source. That is your source's problem.
  The ingest paths redact what they can before storing it and
  `grunnr security redact` cleans the rest, but neither touches your repository
...
- What your MCP client does with the data `grunnr mcp` hands it. That is a
  property of your client and its model provider, not of grunnr.
- Vulnerabilities in `go`, `git`, `node`, `python` or your own test suite,
  which grunnr runs but does not ship.
- Findings from an automated scanner with no demonstrated impact.

## Dependencies

Grunnr is cgo-free by design (`modernc.org/sqlite`), so a release binary
