...
# for `grunnr_edge_<goos>_<goarch>`: install.sh builds the asset name out of
...
# build.sh writes `grunnr_v0.13.0-7-gabc1234_linux_amd64`. Publishing only
...
# the version stamped INSIDE the binary is untouched, so `grunnr version`
...
if [ -z "$DIST" ] || [ -z "$VERSION" ]; then
	grunnr_err "usage: edge-assets.sh --dist DIR --version VERSION"
	exit 2
fi
if [ ! -d "$DIST" ]; then
	grunnr_err "not a directory: $DIST"
	exit 1
fi
...
prefix="grunnr_${VERSION}_"

renamed=0
for src in "$DIST"/grunnr_"$VERSION"_*; do
	[ -e "$src" ] || continue
	base="$(basename "$src")"
	dst="$DIST/grunnr_edge_${base#"$prefix"}"
	if [ -e "$dst" ]; then
		grunnr_err "refusing to overwrite an existing asset: $dst"
		exit 1
	fi
	mv "$src" "$dst"
	printf '  %s -> %s\n' "$base" "$(basename "$dst")" >&2
...
if [ "$renamed" -eq 0 ]; then
	grunnr_err "no assets matched grunnr_${VERSION}_* in $DIST"
	grunnr_err "the edge release would carry names no install path asks for"
	exit 1
fi
