#!/usr/bin/env bash
# Shared helpers for the grunnr build/release scripts.
#
...
# GRUNNR_TARGETS is the shipped matrix. It is a single list, in one place,
...
GRUNNR_TARGETS="linux/amd64 linux/arm64 darwin/amd64 darwin/arm64 windows/amd64 windows/arm64"

# GRUNNR_MODULE is the import path the -X ldflags target.
GRUNNR_MODULE="github.com/sosalejandro/grunnr"

# GRUNNR_COMMANDS is every binary a release ships, in the order they are built.
#
# There are two, and the reason is docs/security.md rather than convenience:
# `grunnr` must not be able to open a socket -- a property enforced by
...
# here.
GRUNNR_COMMANDS="grunnr grunnr-serve"

# grunnr_ldflags_pkg names the package whose Version/Commit/BuildDate the
# stamp writes into, for one command.
#
# They differ because `grunnr-serve` deliberately does NOT import internal/cli:
...
grunnr_ldflags_pkg() {
	case "$1" in
	grunnr) printf '%s/internal/cli\n' "$GRUNNR_MODULE" ;;
	grunnr-serve) printf 'main\n' ;;
	*)
		grunnr_err "grunnr_ldflags_pkg: unknown command $1"
		return 1
		;;
	esac
}
...
# grunnr_date_flavor reports which date(1) dialect is available: "gnu"
...
# GRUNNR_DATE_FLAVOR; unset that to re-probe (the tests do, after shimming
# date on PATH).
grunnr_date_flavor() {
	if [ -n "${GRUNNR_DATE_FLAVOR:-}" ]; then
		printf '%s\n' "$GRUNNR_DATE_FLAVOR"
		return 0
	fi
	local probe="2001-09-09T01:46:40Z" fmt="+%Y-%m-%dT%H:%M:%SZ"
	if [ "$(date -u -d @1000000000 "$fmt" 2>/dev/null)" = "$probe" ]; then
		GRUNNR_DATE_FLAVOR=gnu
	elif [ "$(date -u -r 1000000000 "$fmt" 2>/dev/null)" = "$probe" ]; then
		GRUNNR_DATE_FLAVOR=bsd
	else
		GRUNNR_DATE_FLAVOR=none
	fi
	printf '%s\n' "$GRUNNR_DATE_FLAVOR"
}

# grunnr_epoch_to_iso renders a Unix epoch as RFC 3339 UTC.
grunnr_epoch_to_iso() {
	local epoch="${1:-}"
	case "$epoch" in
	'' | *[!0-9]*)
		grunnr_err "not a decimal Unix epoch: '${epoch}'"
		return 1
		;;
	esac
	local fmt="+%Y-%m-%dT%H:%M:%SZ"
	case "$(grunnr_date_flavor)" in
	gnu) date -u -d "@$epoch" "$fmt" ;;
	bsd) date -u -r "$epoch" "$fmt" ;;
	*)
		grunnr_err "no date(1) able to format a Unix epoch was found on PATH"
		return 1
		;;
	esac
}

# grunnr_source_date_epoch resolves the build timestamp.
#
...
# cross-compiles.
grunnr_source_date_epoch() {
	local root="${1:-.}"
	if [ -n "${SOURCE_DATE_EPOCH:-}" ]; then
		case "$SOURCE_DATE_EPOCH" in
		'' | *[!0-9]*)
			grunnr_err "SOURCE_DATE_EPOCH must be a decimal Unix epoch; got '${SOURCE_DATE_EPOCH}'"
			return 1
			;;
		esac
		printf '%s\n' "$SOURCE_DATE_EPOCH"
	...
	if ! epoch="$(git -C "$root" log -1 --format=%ct 2>/dev/null)" || [ -z "$epoch" ]; then
		grunnr_err "no SOURCE_DATE_EPOCH set and '$root' has no git history to take one from"
		return 1
	fi
	printf '%s\n' "$epoch"
}

# grunnr_resolve_version resolves the version string stamped into the binary.
#
...
# grunnr_resolve_commit returns the 7-char short SHA stamped into the binary,
# matching shortSHALen in internal/cli/buildinfo.go.
grunnr_resolve_commit() {
	local root="${1:-.}" c
	if [ -n "${COMMIT:-}" ]; then
		printf '%s\n' "$COMMIT"
		return 0
...
# grunnr_artifact_name is the released filename for one target. Version first
# so a directory of several releases sorts by version, and the .exe suffix
# only where Windows requires it to be executable.
grunnr_artifact_name() {
	local version="$1" goos="$2" goarch="$3" binary="${4:-grunnr}"
...
# grunnr_compare_trees reports whether two directories hold byte-identical
...
grunnr_compare_trees() {
	local a="$1" b="$2" rc=0 f base da db
	for f in "$a"/*; do
		[ -e "$f" ] || continue
		base="$(basename "$f")"
		if [ ! -e "$b/$base" ]; then
			grunnr_err "missing from second build: $base"
			rc=1
			continue
		fi
		da="$(grunnr_sha256 "$f")"
		db="$(grunnr_sha256 "$b/$base")"
		if [ "$da" != "$db" ]; then
			grunnr_err "NOT reproducible: $base"
			grunnr_err "  build 1: $da"
			grunnr_err "  build 2: $db"
			rc=1
		else
	...
	for f in "$b"/*; do
		...
		base="$(basename "$f")"
		if [ ! -e "$a/$base" ]; then
			grunnr_err "missing from first build: $base"
			rc=1
		fi
	done
	return $rc
...
# grunnr_sha256 prints the bare hex digest of one file. sha256sum (GNU) and
...
grunnr_sha256() {
	...
	if command -v sha256sum >/dev/null 2>&1; then
		...
		grunnr_err "neither sha256sum nor shasum is available"
		return 1
	fi
}

# grunnr_target_is_shipped guards against typos in a GOOS/GOARCH pair. A
# silent success on an unshipped target would produce an artifact nobody
# signs and nobody publishes.
grunnr_target_is_shipped() {
	local want="$1/$2" t
	for t in $GRUNNR_TARGETS; do
		[ "$t" = "$want" ] && return 0
	done
	return 1
}
