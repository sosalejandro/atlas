...
Grunnr reads an entire proprietary codebase and writes what it learns to a
...
**Nothing leaves your machine.** Grunnr makes no outbound network calls: no
upload, no telemetry, no update check, no crash reporting. There is no
account, no server, and no cloud tier today.

That is enforced, not promised, and it is enforced twice because grunnr ships
**two binaries** with different capabilities:

| binary | can it open a socket? | enforced by |
| --- | --- | --- |
| `grunnr` | **No — at all.** It cannot import `net`, `net/http`, `net/rpc` or `net/smtp`. | `TestAtlasBinary_ImportsNoNetworkPackage` walks the import graph from `cmd/grunnr` and fails the build. |
| `grunnr-serve` | It **listens**, on loopback only. It never **dials**. | `TestServeBinary_NeverDialsOut` walks from `cmd/grunnr-serve` looking for the calls — `net.Dial*`, `http.Get/Post/NewRequest`, `http.Client`, `net.Dialer` — and `httpapi.RequireLoopback` refuses any routable address. |

Everything that reads your source lives in `grunnr`, which cannot reach the
network under any circumstances. `grunnr-serve` exists only to hand an
...
`grunnr-serve` has **no authentication** and serves a complete map of the
...
The walk covers the first-party packages `go list -deps ./cmd/grunnr` reports.
...
it for your checkout, run `go list -deps ./cmd/grunnr | grep sosalejandro`.
...
concern tracked in #121; this check is about grunnr's own code.
...
Anyone who receives `grunnr.db` receives all of that. Section 3 lists exactly
which columns.

---

## 2. Every file grunnr writes

| Path | Written by | Contents | Mode |
| --- | --- | --- | --- |
| `.grunnr/grunnr.db` (+ `-wal`, `-shm`) | `init`, `scan`, `snapshot`, `cov`, `sql`, `flow`, `trend record`, and `mcp` on first open | The state database. Section 3. | default |
...
| `$TMPDIR/grunnr-tsscan-*/scanner.ts` (+ a `node_modules` bridge beside it) | the TypeScript scanner | Grunnr's own embedded scanner script, and a symlink (or copy, where symlinks are unavailable) of the TypeScript compiler already installed in your project. | `0600` |

The database path comes from `--db-path`, then `db_path` in `.atlas.yaml`,
then the default `.grunnr/grunnr.db` relative to the repository root.
...
Everything else grunnr produces goes to stdout, to `.grunnr/`, or to a path you
named on the command line.

### Programs grunnr launches

Grunnr chooses to launch these, and nothing else:
...
**Plus whatever you tell it to run.** `grunnr cov run -- <command>` executes
...
collection needs. With no `--` argument it runs `go test ./...`. Grunnr wraps
that process to collect coverage; it does not restrict it. So the accurate
statement is: grunnr launches the four programs above on its own initiative,
and exactly one verb runs a command of your choosing.

Every invocation is argv-style — `exec.Command(name, args...)`. No command
string is ever handed to a shell, so nothing grunnr passes through (a branch
...
Run `grunnr security` for the live version of this, with row counts.
...
| `enum` | A value from a closed vocabulary grunnr assigns itself. |
...
- **`snapshots.index_json`** is written by `grunnr snapshot`, and only by
  `grunnr snapshot` — `grunnr diff` reads snapshot rows, it does not capture
  ...
  `grunnr snapshot`, your doc comments are in the database. If you have not,
  they are not — the `symbols` table has no doc column at all.
- **`sql_operations.sql_text`** exists so `grunnr sql` can tell you a query
...
| `config.key` | Config keys grunnr set. |
...
- Source file *contents*. Grunnr parses files and stores what the parse
...
Grunnr transmits nothing. Everything below is a local file or a local stream,
and what happens to it afterwards is your decision. Run
`grunnr security --export <verb>` to ask about one command.

| Surface | Destination | Carries |
| --- | --- | --- |
| the state database | a local file | everything in §3 |
| `--json` on any verb | stdout | whatever that verb reads — `grunnr sql --json` includes query text, `grunnr flow --json` includes branch conditions |
| `grunnr report sarif` | stdout or `--out` | paths, line spans, rule ids, and messages naming features, symbols and scores. The audit, coverage and dead-code producers quote no source text, so no query text or doc comment reaches this file today |
| `grunnr report github` | stdout, copied by the runner into the job log | the same findings. Job logs are visible to anyone who can see the repository's Actions tab, which on some plans is a wider audience than the repository |
| `grunnr report pr` | stdout; you pipe it to `gh pr comment` | the same findings. Grunnr never calls the GitHub API itself; the comment becomes public the moment it is posted on a public repository |
| `grunnr cov run --out` | one local coverprofile per test | file paths, line ranges, execution counts; the filenames are test symbol names |
| `grunnr cov run --work` | a local directory, temporary unless `--keep` | coverage meta and counter files, and grunnr's plan and report JSON |
| `grunnr mcp` | stdout, as JSON-RPC to the client that launched it | paths, identifiers, coverage figures, **and `features.title`** — which is source text, lifted out of your annotation comments. Capped per tool and read-only by construction |
...
third party. Through your client, never through grunnr.
...
Source contains credentials more often than anyone admits, and grunnr will
...
Grunnr attacks this from both ends.
...
scheme, user and host left intact so `grunnr sql` can still analyse the query.
Each replacement is logged with the file and line the credential is still
sitting in, because rotating it is the part grunnr cannot do for you. The
...
**After the fact.** `grunnr security` scans the stored TEXT columns for four
high-signal shapes:
...
`grunnr security redact` replaces each one with
...
`grunnr sql` then analyses and reports on as though it were your code — a
...
- **The sweep reads the columns in the registry, not every TEXT column in the
  ...
  database, so both `grunnr security` and `grunnr security redact` print a
...
`grunnr security` and `grunnr security redact --dry-run` open the state
...
store captured from a machine running an older grunnr reads back as that older
store.

`grunnr security redact` (without `--dry-run`) is the only one that opens for
...
- There is no authorization model, because there is no multi-user surface.
  Anyone who can read `.grunnr/grunnr.db` can read everything in §3.
