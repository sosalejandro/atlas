...
Embedded via `//go:embed schema/*.sql`. Migration runner:
...
- Migrations are append-only and **immutable** once shipped. Editing an
  applied migration changes its checksum and Atlas refuses to open the store
  (drift detection — see §2 "detect drift").
- No `down.sql` for v1. Rollback = delete `.atlas/state.db` and re-run
  `atlas init`. The store is a cache; the source-of-truth is the code.
- Schema reference docs live in `docs/schema-v1.md` and are regenerated from
  the SQL files by a `docs:schema` task (Phase 7).

This pattern is borrowed wholesale from `bmad-story-runner-cli/infrastructure/state/sqlite/schema/`
— it's known to work and keeps the schema state explicit in one place.
