...
`grunnr snapshot` runs a fresh scan, optionally an audit pass, and persists
the combined view as one row in the `snapshots` table. That row is the
input to [`grunnr diff`](./diff.md) — capture a snapshot on `main` as your
...
| `--note`                      | (empty)               | Free-form note recorded alongside the snapshot. Surfaces in `grunnr diff` headers.                      |
...
$ grunnr snapshot --note "fixture v1"
...
```
# Run from: /tmp/grunnr-fixture
$ grunnr snapshot --note "fixture v2"
...
```
# Run from: any repo
$ grunnr snapshot --include-audit=false --note "structural-only"
snapshot captured  id=3 git_ref=a1b2c3d  note=structural-only
```

A snapshot without the audit slice can still be diff'd — `grunnr diff` will
...
$ grunnr snapshot --ref main --note "nightly-baseline"

# Run from: repo root, on a PR head
$ grunnr snapshot --ref HEAD --note "pr-${PR_NUMBER}"
$ grunnr diff main HEAD   # baseline by git-ref lookup
```

`grunnr diff` accepts the git ref directly — no need to capture the
...
A snapshot is a *frozen* view: subsequent `grunnr scan` / `grunnr cov sync`
