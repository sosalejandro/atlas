...
// TestIngest_VitestFixture is the end-to-end smoke test that exercises
// the full pipeline (vitest parser → coverage.Ingest → store) against
// a committed fixture under packages/coverage/testdata/vitest.json.
// Build-tagged `integration`.
func TestIngest_VitestFixture(t *testing.T) {
	...
	// paths (src/auth/login.test.ts → "auth.login"). Without this the FK
	...
	fixturePath := filepath.Join("..", "testdata", "vitest.json")
	...
	// Wrap the vitest.Parse function into the coverage.Parser interface.
	runID, err := coverage.Ingest(ctx, s, coverage.ParseFunc(vitest.Parse), f, coverage.IngestOptions{})
	if err != nil {
	...
	runs, err := s.Coverage().ListRuns(ctx, store.FrameworkVitest)
	if err != nil {
	...
	got := runs[0]
	if got.Framework != store.FrameworkVitest {
		t.Errorf("CoverageRun.Framework = %q; want %q", got.Framework, store.FrameworkVitest)
	}
